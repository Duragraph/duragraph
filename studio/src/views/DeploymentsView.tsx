...
export function DeploymentsView() {
  ...
  const selected = selectedId ? deployments.find((d) => d.deployment_id === selectedId) : null
        ...
        <div className="flex items-center justify-between border-b border-border bg-card px-6 py-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              ...
              {' · '}
              {assistants?.length ?? 0} assistant{(assistants?.length ?? 0) !== 1 ? 's' : ''} available
            </p>
          </div>
              ...
              {deployments.map((dep) => (
                <tr
                  ...
                  className={`border-b border-border cursor-pointer transition-colors ${
                    selectedId === dep.deployment_id ? 'bg-accent' : 'hover:bg-muted/30'
                  }`}
                >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-block border px-2 py-0.5 text-xs font-medium ${STATUS_STYLES[dep.status] ?? ''}`}
                    >
                      {dep.status}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right font-mono">{dep.workers}</td>
                  <td className="px-4 py-3 text-right font-mono">{dep.active_runs}</td>
                  <td className="px-4 py-3 text-right font-mono">{dep.completed_runs.toLocaleString()}</td>
                  <td className="px-4 py-3 text-right font-mono">{dep.failed_runs}</td>
        ...
        <div className="w-80 border-l border-border bg-card flex flex-col overflow-y-auto">
          ...
          <div className="p-4 space-y-4">
            <div>
              ...
              <div className="mt-1">
                <span
                  className={`inline-block border px-2 py-0.5 text-xs font-medium ${STATUS_STYLES[selected.status] ?? ''}`}
                >
                  {selected.status}
                </span>
              </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="border border-border p-2">
                ...
                <div className="text-lg font-semibold font-mono">{selected.active_runs}</div>
              </div>
              <div className="border border-border p-2">
