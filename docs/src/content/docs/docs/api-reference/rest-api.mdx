...
Rate limit headers are included on responses from the rate-limited routes
(everything except `/health`, `/metrics`, and `/ok`):
...
When the rate limit is exceeded the engine responds with `429 Too Many
Requests` and the following headers:
