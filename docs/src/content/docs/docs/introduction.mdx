...
<Badge text="v0.2.1" variant="success" /> <Badge text="Apache-2.0" variant="note" />
...
```mermaid
flowchart LR
  client["Client SDKs<br/>Python · Go · TypeScript"]
  api[API Layer]
...
DuraGraph implements a compatibility layer with LangGraph Cloud APIs. If you have existing LangGraph workflows, see the [migration guide](/docs/user-guide/tutorials/langgraph-migration) for a step-by-step walkthrough.
