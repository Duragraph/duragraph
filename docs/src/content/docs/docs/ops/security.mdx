...
| **Denial of Service**      | Run flooding, SSE stream spamming | Token bucket rate limiting (`RATE_LIMIT_ENABLED`), backpressure (`429` + `Retry-After`), worker autoscaling |
...
- Configurable rate limiting with standard HTTP headers (`X-RateLimit-Limit`, `X-RateLimit-Remaining`, `Retry-After`).
- Namespace and queue-level workload isolation.
- Optimistic concurrency control and lease epoch fencing for data integrity.
...
- [Deployment](/docs/ops/deployment) — Rate limiting and authentication configuration
- [Security Model](/docs/architecture/security-model) — Architecture-level security design
