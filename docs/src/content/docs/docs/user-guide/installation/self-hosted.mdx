...
DuraGraph publishes multi-arch Docker images (linux/amd64, linux/arm64) to GitHub Container Registry:

```bash
# Latest release
docker pull ghcr.io/duragraph/duragraph:latest

# Specific version
docker pull ghcr.io/duragraph/duragraph:0.2.1
```

See all available tags on the [GitHub Packages page](https://github.com/Duragraph/duragraph/pkgs/container/duragraph).
...
```mermaid
flowchart LR
    A["Client<br/>(SDK/CLI)"] --> B["API Server<br/>(Echo)"]
    B --> C[("PostgreSQL<br/>(Events)")]
    B --> D["NATS<br/>JetStream"]
```
...
services:
  ...
  server:
    image: ghcr.io/duragraph/duragraph:0.2.1
