...
Use multiple guardrails in layers - fast heuristic checks first, then deeper LLM-based checks for flagged content.
</Aside>

<Aside type="caution" title="Avoid Over-Restriction">
Overly strict guardrails can harm user experience. Tune thresholds based on actual false positive rates.
</Aside>
...
<CardGrid>
  <Card title="Trust Framework" icon="approve-check">
    Build [strategic trust](/docs/user-guide/governance/trust-framework) with transparent decision trails
  </Card>
