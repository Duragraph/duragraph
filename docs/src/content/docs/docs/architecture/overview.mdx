...
- **API Server (`cmd/duragraph`)**
  Exposes the DuraGraph REST + SSE API surface (assistants, threads, runs, graphs, stream). Handles request validation, rate limiting, and error responses.
